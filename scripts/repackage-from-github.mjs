...
const manifestUrl = process.env.SOURCE_MANIFEST_URL || "https://raw.githubusercontent.com/warManHy/stock-atlas-data/main/manifest.json";
const publicBaseUrl = process.env.PUBLIC_BASE_URL || "https://raw.githubusercontent.com/warManHy/stock-atlas-data/main";
...
const currentBytes = await getBytes(currentManifest.datasetUrl);
const current = JSON.parse(currentBytes.toString("utf8"));
if (current.schemaVersion !== 3) throw new Error("数据包 schema 与 manifest 不一致");
if (sha256(currentBytes) !== currentManifest.sha256) throw new Error("远端数据包 SHA-256 校验失败");
...
console.log(JSON.stringify({ sourceDataVersion: currentManifest.dataVersion, dataVersion: version, stockCount: current.stockCount, sha256: outputManifest.sha256, bytes: outputBytes.length }));
...
function nextVersion(previous) {
  ...
  const now = Number(`${values.year}${values.month}${values.day}${values.hour}${values.minute}`);
  return Math.max(now, Number(previous) + 1);
}

async function getJson(url) { return JSON.parse((await getBytes(url)).toString("utf8")); }
